"""Bind (or unbind) a real boat's AIS transponder by hand.
    ...
    .venv/bin/python scripts/set_mmsi.py <race_id> <boat name> <mmsi|none>
...
feed matches by name again.  The boat's stored track is kept; run
recompute if a wrong vessel's points crept in (they show as an obviously
wrong track on the map).
"""
import os
import sys
...
def main():
    if len(sys.argv) != 4:
        print(__doc__)
        sys.exit(1)
    race_id, name, arg = int(sys.argv[1]), sys.argv[2], sys.argv[3].strip().lower()
